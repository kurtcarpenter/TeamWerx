...
exports.formTeams = function (roster, teams, min, max) {
  ...
  for (var i = 0; i < finalTeams.length; i++) {
    for (var j = 0; j < finalTeams[i].members.length; j++) {
      currentMembers++
    }
  }

  var avgSize = Math.ceil((min + max) / 2)
  var totalTeams = Math.ceil((1.0 * roster.length + currentMembers) / avgSize)
  var overfilled = 0
  var unmatched = 0
  ...
  logger.info({total: totalTeams, final: finalTeams})
  var existingTeams = finalTeams.length
  for (var i = 0; i < totalTeams - existingTeams; i++) {
    logger.info(i)
    finalTeams.push({
      teamno: i,
      members: [],
